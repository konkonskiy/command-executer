...
class FfmpegExecutor extends command_executor_1.CommandExecutor {
    ...
    prompt() {
        return __awaiter(this, void 0, void 0, function* () {
            const width = yield this.promptService.input('Введите ширину', 'number');
            const height = yield this.promptService.input('Введите высоту', 'number');
            const path = yield this.promptService.input('Введите путь до файла', 'input');
            const name = yield this.promptService.input('Введите название файла', 'input');
            return { width, height, path, name };
        });
    }
    bild({ width, height, path, name }) {
    ...
    spawn({ command, args, output }) {
        this.fileService.deleteFailIfExists(output);
